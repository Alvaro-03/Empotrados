...
// Función para crear una matriz y llenarla con valores aleatorios entre 0 y 1 (thread-safe)
template <typename T>
Matriz<T> crearMatrizAleatoria(size_t filas, size_t columnas) {
    // Inicializar la semilla para la generación de números aleatorios
    ...
    for (size_t i = 0; i < filas; ++i) {
        matriz.datos[i] = new T[columnas];
        for (size_t j = 0; j < columnas; ++j) {
            seed = rand_r(&seed); // Utilizar rand_r para hacer la generación de números aleatorios thread-safe
            matriz.datos[i][j] = static_cast<T>(seed) / RAND_MAX;
        }
    }
